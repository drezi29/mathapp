...
class FormulaNote(models.Model):
    ...
    note = models.TextField(verbose_name=_('formula note'), help_text=_('Provide note for formula card chapter'))
    order = models.IntegerField(blank=False, verbose_name=_('order'),
                                help_text=_('The value determines the order of notes in formula\'s chapter view'))
    ...
    class Meta:
        constraints = [models.UniqueConstraint(
            fields=['title', 'order'],
            name='unique_order_for_note_in_chapter')
        ]
